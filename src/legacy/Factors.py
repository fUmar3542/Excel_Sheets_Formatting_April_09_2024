...
def calculate_position_betas(
    ...
    try:
        ...
        MyExceptions.show_message(tab='Factors.py',
                                  message="Following exception occurred during calculating position's beta\n\n" + str(
                                      ex))
    ...
    try:
        ...
        MyExceptions.show_message(tab='Factors.py',
                                  message="Following exception occurred during merging of factor returns and position returns\n\n" + str(
                                      ex))

    try:
        # natural beta vs factors
        covar_position_fact = mr.groupby(['position', 'factor'])[['position_return', 'factor_return']].cov()
        var_df = covar_position_fact.iloc[1::2, 1].reset_index().drop(columns=['level_2']).set_index(
            ['position', 'factor'])
        covar_df = covar_position_fact.iloc[::2, 1].reset_index().drop(columns=['level_2']).set_index(
            ['position', 'factor'])
        ...
        MyExceptions.show_message(tab='Factors.py',
                                  message="Following exception occurred during calculation of SLR beta\n\n" + str(
                                      ex))
    try:
        # get Equity returns
        factor_eq = factor_returns.columns[0]  # the Equity Factor is always the first column, cf helper.imply_smb_gmv
        fact_eq = mr[mr['factor'] == factor_eq][['date', 'factor', 'factor_return']].drop_duplicates().rename(
            columns={"factor": factor_eq, "factor_return": "eq_return"})
        mr = pd.merge(mr, fact_eq, on='date', how='left')
    except Exception as ex:
        MyExceptions.show_message(tab='Factors.py',
                                  message="Following exception occurred during getting equity returns\n\n" + str(
                                      ex))
    try:
        ...
        MyExceptions.show_message(tab='Factors.py',
                                  message="Following exception occurred during calaculation of beta returns vs equity\n\n" + str(
                                      ex))
    try:
        ...
        MyExceptions.show_message(tab='Factors.py',
                                  message="Following exception occurred during adjusting factor returns\n\n" + str(
                                      ex))
    try:
        ...
        MyExceptions.show_message(tab='Factors.py',
                                  message="Following exception occurred during merging of beta series\n\n" + str(
                                      ex))
    finally:
