...
class GroupVarCalculator:
    ...
    def __post_init__(self):
        # make sure that prices table consists only of factors
    ...
    def _group_positions(self) -> pd.DataFrame:
        ...
        """
        return self.positions_table.groupby([self.group_column, "VaRTicker"]).agg(
            {"Exposure": "sum"}
        )
    ...
    def _group_weights(self) -> pd.Series:
        ...
        Name: Weight, dtype: float64
        """
        #return self._group_positions.Exposure / self._group_positions.Exposure.sum()  #XM: this is wrong: exposure is Value of assets / NAV 
        return self._group_positions.Exposure / self.nav
    ...
    def _iso_fund_stds(self) -> pd.Series:
        ...
        for group in groups:
            ...
            group_return_covariance = (
                self.portfolio_var_calculator.return_covariance.loc[
                    group_positions, group_positions
                ]
            )
            fund_std = (
                group_exposure.T @ group_return_covariance @ group_exposure
            ) ** 0.5
    ...
    def component_var(self, quantile) -> pd.Series:
        ...
        return self.portfolio_var(quantile) * fund_weights
    ...
    def _group_incremental_var(self, group, quantile) -> pd.Series:
        """returns incremental var for given quantile"""
        return self.portfolio_var(quantile) - self._isolated_group_calculators[
            group
