...
def handle_files(investment_advisor, today, yaml_file):
    ...
    try:
        ...
        MyExceptions.show_message(tab='main.py',
                                  message="Following exception occurred during the creation of the directory for output files.\n\n" + str(
                                      ex))
    ...
    try:
        ...
        MyExceptions.show_message(tab='main.py',
                                  message="Following exception occurred during copying the configuration file.\n\n" + str(
                                      ex))


def read_nav_history(investment_advisor, holdings_date):
    AUM_dict = {}
    AUM_clean_df_list = []
    AUM_clean = None
    AUM_clean_funds = None
    try:
        # reads NAV history
        AUM = pd.read_excel(file_paths["historical_pnl_nav"].format(investment_advisor=investment_advisor,
                                                                    holdings_date=pd.to_datetime(
                                                                        holdings_date).strftime(
                                                                        '%Y%m%d')))  # read path from yaml file
...
def load_global_data():
    ...
    VAR_NBDAYS_AGO = None
    RISK_FREE_RATE = None
    COLNAME_POSITION_GROUPING = None
    DECAY_FACTOR = None
    QUANTILES = None
    OPTION_TYPES = None
    NON_OPTION_TYPES = None
    FUND_NAME_ALIAS = None
    price_vol_shock_range = None
    file_paths = None
    INVESTMENT = None
    ...
    try:
        ...
        if global_settings:
            ANNUALIZATION_FACTOR = global_settings.get("annualization_factor", 252)
            VAR_NBDAYS_AGO = global_settings.get("var_nbdays_ago", 182)
            RISK_FREE_RATE = global_settings.get("risk_free_rate", 0.055)
            COLNAME_POSITION_GROUPING = global_settings.get("column_position_grouping", "UnderlierName")
            DECAY_FACTOR = global_settings.get("decay factor", 0.94)
            QUANTILES = global_settings.get("quantiles", {})
            OPTION_TYPES = global_settings.get("option types", {})
            NON_OPTION_TYPES = global_settings.get("non option types", {})
            FUND_NAME_ALIAS = global_settings.get("fund name alias", "")
            price_vol_shock_range = global_settings.get("stress_test_ranges", {})

        fund_settings = config.fund_settings
        if fund_settings:
            INVESTMENT = fund_settings[0].get("investment_advisor")

        file_paths = config.global_settings.get("file_paths", {})
    except Exception as ex:
        MyExceptions.show_message(tab='main.py',
                                  message="Following exception occurred during reading configuration file\n\n" + str(
                                      ex))
    finally:
        return ANNUALIZATION_FACTOR, VAR_NBDAYS_AGO, RISK_FREE_RATE, COLNAME_POSITION_GROUPING, DECAY_FACTOR, QUANTILES, OPTION_TYPES, NON_OPTION_TYPES, FUND_NAME_ALIAS, price_vol_shock_range, file_paths, INVESTMENT


def extract_fund_info():
    investment_advisor = None
    fund_list = None
    bench_names = None
    holdings_date = None
    FACTORS_TO_REPORT = None
    custom_outputs = None
    try:
        ...
        MyExceptions.show_message(tab='main.py',
                                  message="Following exception occurred during reading fund settings from configuration file\n\n" + str(
                                      ex))
    finally:
        return investment_advisor, fund_list, bench_names, holdings_date, FACTORS_TO_REPORT, custom_outputs


def read_and_process_factor_prices_position(investment_advisor, holdings_date):
    factor = None
    factor_prices = None
    factor_betas = None
    raw_factor_betas = None
    position = None
    position_returns = None
    filters_dict = None
    matrix_cov = None
    raw_matrix_cov = None
    position_normed_prices = None
    ...
    try:
        ...
        eq_factor_name = factor.loc[factor['Factor Names'] == 'Equity', 'FactorID'].values[
            0]  # name of the main Equity Factor (benchmark)
        price = pd.read_csv(file_paths["prices"].format(investment_advisor=investment_advisor,
        ...
        factor_ids_price = [col for col in price.columns if
                            not (col.startswith('BBG') or col.startswith('Unnamed'))]
        ...
        # beta estimation
        factor_betas, factor_returns_ortho, raw_factor_betas = fac.calculate_position_betas(factor_returns,
                                                                                            position_returns)

        logger.info("Correlations, Covariances ")
        matrix_correlation = factor_returns_ortho.rename(
            columns=dict(zip(factor_returns_ortho.columns, factor['Factor Names']))).corr()
        ...
        MyExceptions.show_message(tab='main.py',
                                  message="Following exception occurred during reading and processing of factors/prices/positions\n\n" + str(
                                      ex))
    finally:
        return factor, factor_prices, factor_betas, raw_factor_betas, position, position_returns, filters_dict, matrix_cov, raw_matrix_cov, position_normed_prices, matrix_correlation


def apply_filters_to_input_data(fund, AUM_dict, AUM_clean_df, factor, factor_prices, factor_betas, raw_factor_betas, position,
                                position_returns, filters_dict, matrix_cov, raw_matrix_cov, position_normed_prices):

    sector_factor_decomp_df = top_var_withexpo = top_vardiv_withexpo = sector_exposure_df = options_premium_df = greek_sensitivities_df = None
    fund_exp_pct_dashboard = fund_exp_usd_dashboard = position_liquidity = factor_heat_map = beta_heat_map = macro_factor_decomp_df = None
    risk_factor_exposure_top_N_list = risk_factor_exposure_bottom_N_list = macro_beta_decomp_df = sector_beta_decomp_df = risk_beta_exposure_top_N_list = None
    risk_beta_exposure_bottom_N_list = analyst_exposure_df = analyst_beta_adj_exposure_df = strat_exposure_df = strat_beta_adj_exposure_df = None
    assettype_exposure_df = assettype_beta_adj_exposure_df = sector_beta_adj_exposure_df = industry_exposure_df = industry_beta_adj_exposure_df = country_exposure_df = None
    country_beta_adj_exposure_df = mktcap_exposure_df = mktcap_beta_adj_exposure_df = top_var_contributors = top_var_diversifiers = var_structured_analyst = None
    var_structured_assettype = var_structured_strat = var_structured_sector = var_structured_industry = var_structured_country = var_structured_mcap = None
    options_delta_adj_exposure_df = options_delta1_exposure_df = stress_test_price_vol_results_df = stress_test_beta_price_vol_results_df = None
    stress_test_price_vol_exposure_results_df = stress_test_filtered_df = options_delta_df = position_summary = position_breakdown = None
    try:
        filter_items = {
            "position": "VaRTicker",
            "fund": "Strat",
        ...
        var_data = calculate_vars(prices=position_normed_prices, positions=position,
                                  nav=nav_value)  # price.iloc[price.index>=date_min_var,:]
        ...
        top_var_withexpo = top_var_contributors.merge(right=curpos, left_index=True, right_index=True,
                                                      how='left')
        top_vardiv_withexpo = top_var_diversifiers.merge(right=curpos, left_index=True, right_index=True,
                                                         how='left')
    except Exception as ex:
        MyExceptions.show_message(tab='main.py',
                                  message="Following exception occurred during applying filters to the input data\n\n" + str(
                                      ex))
    finally:
        return sector_factor_decomp_df, top_var_withexpo, top_vardiv_withexpo, sector_exposure_df, options_premium_df, greek_sensitivities_df, \
                fund_exp_pct_dashboard, fund_exp_usd_dashboard, position_liquidity, factor_heat_map, beta_heat_map, macro_factor_decomp_df, \
                risk_factor_exposure_top_N_list, risk_factor_exposure_bottom_N_list, macro_beta_decomp_df, sector_beta_decomp_df, risk_beta_exposure_top_N_list, \
                risk_beta_exposure_bottom_N_list, analyst_exposure_df, analyst_beta_adj_exposure_df, analyst_beta_adj_exposure_df, strat_exposure_df, strat_beta_adj_exposure_df, \
                assettype_exposure_df, assettype_beta_adj_exposure_df, sector_beta_adj_exposure_df, industry_exposure_df, industry_beta_adj_exposure_df, country_exposure_df, \
                country_beta_adj_exposure_df, mktcap_exposure_df, mktcap_beta_adj_exposure_df, top_var_contributors, top_var_diversifiers, var_structured_analyst, var_structured_assettype, \
                var_structured_strat, var_structured_sector, var_structured_industry, var_structured_country, var_structured_mcap, options_delta_adj_exposure_df, options_delta1_exposure_df, \
                stress_test_price_vol_results_df, stress_test_beta_price_vol_results_df, stress_test_price_vol_exposure_results_df, stress_test_filtered_df, options_delta_df, position_summary, \
               position_breakdown, fund, AUM_dict, AUM_clean_df, factor, factor_prices, factor_betas, raw_factor_betas, position, position_returns, filters_dict, matrix_cov, raw_matrix_cov, position_normed_prices


if __name__ == "__main__":  # Execution of main procedure for laumch in command line

    # Load global data
    ANNUALIZATION_FACTOR, VAR_NBDAYS_AGO, RISK_FREE_RATE, COLNAME_POSITION_GROUPING, DECAY_FACTOR, QUANTILES, OPTION_TYPES, NON_OPTION_TYPES, FUND_NAME_ALIAS, price_vol_shock_range, file_paths, INVESTTMENT = load_global_data()

    # Extract fund-specific information
    investment_advisor, fund_list, bench_names, holdings_date, FACTORS_TO_REPORT, custom_outputs = extract_fund_info()
    # Define the new filename
    today = datetime.today().strftime('%Y%m%d')  # Format: YYYYMMDD
    # Create output files
    handle_files(INVESTTMENT, today, yaml_file)
    ...
    # read nav history
    AUM_dict, AUM_clean, AUM_clean_funds = read_nav_history(investment_advisor, holdings_date)
    # get all keys
    fund_list_aum = list(AUM_dict.keys())
    # reads benchmarks price history 
    df_bench = pd.read_excel(file_paths["benchmarks_histo"].format(investment_advisor=investment_advisor),
                             sheet_name="Sheet1", header=0)  # load from yaml
    ...
    try:
        for fund in fund_list_aum:
            ...
            try:
                ...
                MyExceptions.show_message(tab='main.py',
                                          message="Following exception occurred during creation of output file\n\n" + str(
                                              ex))
            ...
            factor, factor_prices, factor_betas, raw_factor_betas, position, position_returns, filters_dict, matrix_cov, raw_matrix_cov, position_normed_prices, matrix_correlation = read_and_process_factor_prices_position(investment_advisor, holdings_date)
            ...
            if len(position) == 0:
                ...
                sector_factor_decomp_df, top_var_withexpo, top_vardiv_withexpo, sector_exposure_df, options_premium_df, greek_sensitivities_df, \
                fund_exp_pct_dashboard, fund_exp_usd_dashboard, position_liquidity, factor_heat_map, beta_heat_map, macro_factor_decomp_df, \
                risk_factor_exposure_top_N_list, risk_factor_exposure_bottom_N_list, macro_beta_decomp_df, sector_beta_decomp_df, risk_beta_exposure_top_N_list, \
                risk_beta_exposure_bottom_N_list, analyst_exposure_df, analyst_beta_adj_exposure_df, analyst_beta_adj_exposure_df, strat_exposure_df, strat_beta_adj_exposure_df, \
                assettype_exposure_df, assettype_beta_adj_exposure_df, sector_beta_adj_exposure_df, industry_exposure_df, industry_beta_adj_exposure_df, country_exposure_df, \
                ...
                var_structured_strat, var_structured_sector, var_structured_industry, var_structured_country, var_structured_mcap, options_delta_adj_exposure_df, options_delta1_exposure_df, \
                stress_test_price_vol_results_df, stress_test_beta_price_vol_results_df, stress_test_price_vol_exposure_results_df, stress_test_filtered_df, options_delta_df, position_summary, \
                position_breakdown, fund, AUM_dict, AUM_clean_df, factor, factor_prices, factor_betas, raw_factor_betas, position, position_returns, \
                    filters_dict, matrix_cov, raw_matrix_cov, position_normed_prices = apply_filters_to_input_data(
                    fund, AUM_dict, AUM_clean_df, factor, factor_prices, factor_betas, raw_factor_betas, position, position_returns,
                    filters_dict, matrix_cov, raw_matrix_cov, position_normed_prices)
        ...
        MyExceptions.show_message(tab='main.py',
                                  message="Following exception occurred during processing the data from input files\n\n" + str(
                                      ex))
